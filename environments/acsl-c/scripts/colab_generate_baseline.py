"""Generate deterministic Qwen completions for later verifier scoring.

This script deliberately does not execute generated C. It records raw outputs,
task IDs, model revision, and decoding settings so a Docker/Frama-C replay can
score the exact same completions later.
"""
...
def main() -> int:
    ...
    if not records:
        raise SystemExit(
            ...
            "canonical CASP JSONL containing the split IDs (for this repo, "
            "validation/test records are in casp_eval.jsonl)."
        )
    import torch
    ...
    with args.output.open("w", encoding="utf-8") as stream:
        for index, record in enumerate(records):
            ...
            stream.flush()
            print(json.dumps({k: row[k] for k in row if k != "completion"}), flush=True)
            del inputs, output
    return 0
